...
export async function updateme(req, res, next){
    ...
    // 사용자 정보 업데이트
    await userRepository.updateUser(id, { username, gender, age, phnumber });
...
export async function searchuserid(req, res, next){
    ...
    try {
        ...
        // 에러 처리
        res.status(500).json({ message: 'server에 오류가 발생했습니다', error });
    }
}
...
export async function findPassword(req, res, next){
    ...
    try {
        ...
        // 사용자 객체에서 저장된 핸드폰 번호를 가져옴
        const storedPhNumber = user.phnumber;

        // 제출된 핸드폰 번호와 저장된 핸드폰 번호를 비교
        ...
        res.status(500).json({ message: 'server에 오류가 발생했습니다.', error: error.toString() });
    }
}
...
export async function changePassword(req, res) {
    ...
    try {
        ...
        console.error(error);
        res.status(500).json({ message: '비밀번호 변경 중 오류가 발생 했습니다.', error: error.toString() });
    }
}